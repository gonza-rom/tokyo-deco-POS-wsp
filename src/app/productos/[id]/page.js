      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: `1px solid ${C.arenaBorder}`, marginBottom: 20 }}>
        {tabs.map((tab, i) => (
          <button key={tab} onClick={() => setActive(i)} style={{
            ...
            borderBottom: active === i ? `2px solid ${C.terracota}` : '2px solid transparent',
            color: active === i ? C.terracota : C.muted,
...
function ValoresSection({ producto }) {
  return (
    <section style={{ margin: '64px 0', padding: '48px', background: C.carbon }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 40 }}>
        <div style={{ width: 32, height: 1, background: C.arenaText }} />
        <span style={{ fontFamily: 'Georgia, serif', fontSize: 14, fontStyle: 'italic', color: C.arenaText }}>hecho a mano</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 40 }}>
        {[
          ...
          <div key={title}>
            <Icon size={22} color={C.terracota} style={{ marginBottom: 12 }} />
            <h3 style={{ fontFamily: 'Georgia, serif', fontSize: 16, color: C.crema, marginBottom: 8 }}>{title}</h3>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: 'rgba(250,249,247,0.5)', lineHeight: 1.7, margin: 0 }}>{desc}</p>
          </div>
        ))}
      </div>
    </section>
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: C.crema, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: 40, height: 40, border: `2px solid ${C.arenaBorder}`, borderTopColor: C.terracota, borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 16px' }} />
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 24px' }}>
                ...
                {producto.stock > 0 ? (
                  <>
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      style={{ width: '100%', padding: '16px', background: agregado ? C.salvia : C.carbon, color: C.crema, border: 'none', fontFamily: 'Inter, sans-serif', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.2em', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, transition: 'background 0.2s' }}
                    >
                      <ShoppingBag size={16} />
                    ...
                    <button onClick={handleWhatsApp} style={{ width: '100%', padding: '14px', background: C.salvia, color: C.crema, border: 'none', fontFamily: 'Inter, sans-serif', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.2em', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
        ...
        {productosRelacionados.length > 0 && (
          <section style={{ marginBottom: 64 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 40 }}>
              <div style={{ width: 32, height: 1, background: C.arenaText }} />
              <span style={{ fontFamily: 'Georgia, serif', fontSize: 14, fontStyle: 'italic', color: C.arenaText }}>también te puede gustar</span>
            </div>
